...
| D-05 | `29_Permissions...` §3 (staged authorization) | The Calendar OAuth scope (`https://www.googleapis.com/auth/calendar`) is **declared in `appsscript.json`** once CalendarService ships | With an explicit `oauthScopes` list, Apps Script does not auto-add scopes on first use, so the scope must be declared or CalendarApp fails. "Staged" is honored at the UX level: the user only authorizes/uses Calendar when they enable it (Menu ▸ Connect Calendar), and Calendar features do nothing until then. The scope is not present in the M1/M2 manifests. | ADR-018 |
